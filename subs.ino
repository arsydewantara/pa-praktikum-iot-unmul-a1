...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  if (level > 600) { // pengkondisian
    snprintf (msg, MSG_BUFFER_SIZE, "%s", itoa(1,msg,10)); // itoa (konversi integer ke string)
    client.publish("iot_unmul/iot_a_3/signal_led_merah", msg); // fungsi untuk publish ke broker

    snprintf (msg, MSG_BUFFER_SIZE, "%s", itoa(0,msg,10)); // itoa (konversi integer ke string)
    client.publish("iot_unmul/iot_a_3/signal_led_kuning", msg); // fungsi untuk publish ke broker

    snprintf (msg, MSG_BUFFER_SIZE, "%s", itoa(1,msg,10)); // itoa (konversi integer ke string)
    client.publish("iot_unmul/iot_a_3/signal_buzz", msg); // fungsi untuk publish ke broker
...
void reconnect() {
  ...
  while (!client.connected()) {
    ...
    if (client.connect(clientId.c_str())) {
      ...
      client.subscribe("iot_unmul/iot_a_3/air");
    } else {
